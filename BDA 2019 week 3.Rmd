...
packs = c('ggplot2', 'LSD','dplyr','tidyr','scales','knitr', 'readr', 'devtools', 'cowplot')
...
# load the data
load('~/Dropbox/BDA-2019/bda_week3.RData')
```
...
plot(mydata2$x, mydata2$y) # bit ugly
...
We can also fit a line to te x/y data
...
ggplot(mydata2,
  ...
  geom_smooth(color = 'red', # geom_smooth has many parms to adjust, see the documentation for more details
              ...
              se = F) # remove the Standard Error bars for a cleaner visualization - but you might want them for your assignments
...
my.results = error_bars(mydata2,
                       draw = 'both') #first, create the error_bars.out object

# NOTE:
# you need to pass the error_bars function a single dataframe - and it MUST be formatted as data.frame(x,y). If you have a dataframe with more variables then you need to make a new one to use the error_bars function. This is quite easy to do, e.g.:
...
ggplot(df.summary,
       aes(xmean, ymean)) +
  geom_point(size = 2) + # each point shows a bin - with the mean x value and the mean y value
  geom_errorbar(aes(ymin = ymean_minus_sd,
  ...
  geom_smooth(color = 'red', method = 'lm', se = F) + # fit a line to the biined values
...
ggplot(df.summary,
  ...
  geom_errorbar(aes(ymin = ymean_minus_sd,
                    ymax = ymean_plus_sd,
                    width = 0.75)) + # same error bars as above
  geom_smooth(color = 'red',
              method = 'lm',
              se = F) + # fitted line for the binned values in red (as we had before)
...
summary(fit.log)$r.squared # 0.78 - far higher than in the linear model (which was 0.31)
